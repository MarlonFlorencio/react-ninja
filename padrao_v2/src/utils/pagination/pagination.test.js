...
test('pagination({ total: "abc", activePage: 1 }) should thorw an error', () => {
  try {
    pagination({ total: 'abc', activePage: 1 })
  } catch (e) {
    expect(e.message).to.be.equal('total should be a number')
  }
})

test('pagination({ total: 5, activePage: "abc" }) should thorw an error', () => {
  try {
    pagination({ total: 5, activePage: 'abc' })
  } catch (e) {
